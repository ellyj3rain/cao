...
|---|---|---|
| Culture question registry | **PASS** | 48 unique registry-3 questions |
| Culture decision categories | **PASS** | all 12 player decision categories represented |
| Question contracts | **PASS** | 48/48 have five anchors, an executable evidence, feedback, or behavior route, history, and research provenance |
| B16 research and mechanic decomposition | **PASS** | 24/24 added questions bind a named source family, a playable mechanical seam, an ownership decision, and a downstream consumer in the governed corpus |
| Executable question routes | **PASS** | autonomous-behavior+native-doctrine+repeated-practice+social-fact=1; autonomous-behavior+native-doctrine+social-fact=3; autonomous-behavior+repeated-practice+social-fact=2; autonomous-behavior+social-fact=1; native-doctrine+repeated-practice=14; native-doctrine+repeated-practice+represented-state=2; native-doctrine+repeated-practice+social-fact=1; native-doctrine+represented-state=9; repeated-practice=1; represented-state=5; social-fact=9 |
| Behavior consumer coverage | **PASS** | 7/48 questions currently affect autonomous behavior; remaining questions are observable appraisal or feedback state, not falsely counted as behavior consumers |
| Doctrine is evidence, not a consumer | **PASS** | 30 questions accept exact doctrine pressure without counting it as downstream execution |
| Question perturbation uses production materialization | **PASS** | 48/48 fixed-seed question positions change through CACultureDistributionKernel |
| Culture preset completion | **PASS** | 22 complete in-range presets carry contextual sources, explicitly low-confidence per-question estimates, and selected intended contrasts; no fitted historical measurement is claimed |
| Doctrine pressure preserves disagreement | **PASS** | center, intensity, conflict, spouse plurality, and sex-asymmetric spouse pressure remain distinct |
| Many-to-many signed subject mappings | **PASS** | execution, slaughter, and research reach every declared question; unknown subjects reach none |
| Registry-2 Culture migration | **PASS** | rows=48; registry=3; legacy rows exact=True |
| Incomplete registry-2 Culture rejection | **PASS** | a predecessor scope must contain every registry-2 question; migration appends only the 24 B16 questions and never neutral-fills missing historical state |
...
| Semantic definitions exist | **PASS** | every supported adapter resolves in its authoritative Def corpus |
| Unknown mod doctrine fails closed | **PASS** | an unknown package cannot inherit a known defName's semantics |
| Target-specific doctrine remains native-only | **PASS** | animal-veneration and fishing prescriptions do not masquerade as a global animal moral-standing appraisal |
| Exact Ideoligion source-label resolution | **PASS** | a registered package\|kind\|def token resolves its native label; a package mismatch and retired shorthand both fail closed |
| No semantic name guessing | **PASS** | production matches exact package and Def identity only |
| Exact native-practice registry | **PASS** | 54 exact native events resolve to represented practices |
| Unknown native event fails closed | **PASS** | unknown events stay native and uninterpreted |
| Practice contracts | **PASS** | 55 production practice definitions validate |
| Cannibalism ownership separation | **PASS** | native doctrine, Culture appraisal, and actual practice remain three facts |
| Native executor remains authoritative | **PASS** | CA observes supported HistoryEvents after RimWorld executes them; non-blocking prefixes only scope audited native acts |
| Native event emitter manifest | **PASS** | 54 registered events each name exactly one audited emitter contract |
| Native event emission source evidence | **PASS** | direct=44; ingestible definition=1; locally scoped direct=1; observed target=2; participant event in one settlement abandonment=1; selected native event=5; every route binds a declaring method, exact event-token count, and one containing-call or identifier-correlated local-dataflow contract; 16 positive and negative mutation guardrails reject code-hidden, prefixed, neighboring, misplaced, unrelated, and unbound evidence |
| Native occurrence identity and locality | **PASS** | participants collapse within one act, two same-tick acts remain distinct, a process reload at the same tick retains a different session identity, observers collapse per target, and missing shared identity fails closed |
| Intimacy occurrence identity | **PASS** | two participant emissions from one encounter remain one occurrence; a later encounter with new native jobs becomes a second occurrence |
| Proxy Doer is not attributed | **PASS** | SoldSlave is accepted only inside the audited local trade scope; the remote random-colonist emitter has no act identity and is ignored |
...
This is executable/static evidence. Operator visual and gameplay acceptance remains the runtime boundary.
