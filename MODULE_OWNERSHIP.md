...
Scope: active CAO source at the B15 faction-owned Technological Knowledge boundary
...
| `authoring.ontology` | Semantic-kind contracts, production social-referent, admitted twenty-four-question Culture and concrete-practice vocabulary, complete Political Order questions/presets, Technological Knowledge domains/competencies/profiles, represented-institution comparison, category cardinality policy, and control-to-consumer contracts | No persistent state; immutable production registries and copy-on-apply profile kernels | Authoring projection, validation, complete preset application, deterministic generation, and derived identity/account | represented mechanics + model owners → Culture, Political Order, and Technological Knowledge composers, coverage, and receipts | `AuthoringOntologyProjection`; the current Culture registry is not an exhaustive playable-mechanics audit; additions require a coherent construct, production source, consumer, distinct conduct or mechanism, and coverage receipt |
