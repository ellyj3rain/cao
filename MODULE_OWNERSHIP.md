...
Scope: active CAO source at the B17 site-affiliation and pawn-epistemics boundary
...
| `authoring.ontology` | Semantic-kind contracts, production social-referent, forty-eight-question Culture and concrete-practice vocabulary, complete Political Order questions/presets, Technological Knowledge domains/competencies/profiles, represented-institution comparison, category cardinality policy, and control-to-consumer contracts | No persistent state; immutable production registries and copy-on-apply profile kernels | Authoring projection, validation, complete preset application, deterministic generation, and derived identity/account | represented mechanics + model owners → Culture, Political Order, and Technological Knowledge composers, coverage, and receipts | `AuthoringOntologyProjection`; forty-eight questions in twelve categories is the current reverse-audit result rather than a quota; additions require a coherent construct, production source, consumer, distinct conduct or mechanism, and coverage receipt |
| `society.authoring` | Joint control surface plus twenty-two built-in and reusable user presets, each with independent Society identity and metadata and complete Culture, Political Order, and Technological Knowledge snapshots | `CASocietyPresetLibrary` owns validation and rollback-safe atomic copy-on-apply; `CAUserSocietyProfile` persists deep three-component snapshots only in global settings, outside campaign state | Player founding, established-faction authoring, or new-society placement in Starting Region | existing Society section → canonical faction `CACulture`, `CAPoliticalBeliefs`, and `CATechnologicalKnowledge` → separate editors and consumers; Starting Region separately creates ordinary faction, settlement, population-source, and map-assignment state | Society is a control surface, not a state owner; all three components validate before any canonical component changes, an exceptional commit restores all three, and a preset never writes Ideoligion, founding terms, represented institutions, settlement type, or event history |
...
| `ideoligion.semantics` | Exact semantic mappings from native or supported-mod memes and precepts to bounded Culture evidence | No persistent state; `CAIdeoligionSemanticAdapterRegistry` owns immutable package-kind-defName mappings while RimWorld remains the doctrine owner and executor | On exact loaded Ideoligion facts during represented Culture derivation or reconciliation | native Ideoligion definition identity → Culture evidence pressure; no direct Culture replacement | `IdeoligionSemanticAdapter`; labels and defName similarity never imply meaning, and unknown definitions remain native-only |
...
| `culture.native-practice` | Exact native `HistoryEventDef` occurrence interpretation for potentially repeated conduct | No separate owner; `CANativeCultureEventAdapterRegistry` owns immutable package-event mappings and writes bounded occurrence provenance through the map Culture-longitudinal owner after native execution | Harmony postfix after exact native event execution | native event plus actor/faction/map/tick provenance → occurrence ledger → repeated-practice evidence | `NativeCulturePractice`; one event is not a practice, occurrence is not approval, global events without actor provenance are not attributed, and unknown definitions remain native-only |
| `culture.longitudinal` | Local lived Culture question distributions, concrete practices, observations, exact native occurrences, legacy evidence and transition history | `CACultureLongitudinalMapComponent` schema 3; direct-question measurements persist in the owning `CACulture` observation ledger and transition kernels run only from its evaluation | Native occurrence ingress plus daily, map-staggered evaluation | reactions, exact native events, population, programs, organizations → cultural cognition and history | `CultureLongitudinalUpdate`; direct measurements and native occurrences retain exact provenance; only repeated occurrence evidence can establish practice and new evidence adapters remain pure |
...
| `proposition.knowledge` | Propositions plus optional pawn-private typed facts, holders, sources, immediate reporters, event/acquisition time, channels, contradictions, revisions, supersession, access, confidence, uncertainty, scope, transmission, custody, retention and research receipts | `CAPropositionKnowledgeWorldComponent`; observation/report/research and represented-fact ingress plus owning knowledge ticks | Event ingress; broader-mode source sync and relay; 60,000-tick native/institution sync; fact-specific retention and 600,000-tick decay | direct observation, testimony, represented sites and people, native research, settlement research, organizations and communications → inspection, cognition, politics and behavior | `PropositionKnowledge`; broader mode defaults off, relays copy the teller's record rather than live truth, contradictory records can coexist, direct observation bypasses access norms, and novelty never determines truth |
| `technology.knowledge` | Nine practical domains, understand/construct/operate/maintain competencies, native research translation, known-project evidence, and optional pawn/institution/record custody | A faction owns its canonical `CATechnologicalKnowledge`; a factionless or explicitly divergent site owns its local copy in `CASiteLocalSocietyState`; `CATechnologicalKnowledgeRuntime` is the sole availability/query boundary | Authoring and faction/site creation; native research completion; distributed-mode initial distribution, usable humanlike member spawn/faction change, death, incapacity query, and game start/load initialization | effective faction or site state + native research/buildable/manufactured-item/recipe/plant definitions + custody → current consumers enumerated in `Receipts/B15/B15_TECHNOLOGICAL_KNOWLEDGE_CONSUMER_MATRIX.md` | `TechnologicalKnowledge`; standard mode reads the effective social owner, distributed mode projects the same state through accessible custody, teaching is not yet implemented, and no settlement tier or `FactionDef` becomes a second owner |
| `faction.state` | Established faction Culture, normative Political Order, Technological Knowledge, and represented institutions | `CAFactionStateWorldComponent`; ensure/find and explicit authoring/materialization paths | World creation/load and represented changes | faction authoring + native factions → regional settlements, Culture, politics, research, production, habitat, and autonomous development | compatibility owner receipt; Society only composes the three canonical components, while Political Order and represented institutions retain separate writes and consumers |
| `player.founding` | Confirmed founders' staged Culture, Ideoligion link, Political Order, Technological Knowledge, and adopted founding arrangement | `CAPlayerFoundingWorldComponent`; confirmation/materialization only, followed by one copy into the realized player faction | Colony creation, one founding application, then historical runtime state | creation flow → canonical faction state and organization | founding receipts; the faction becomes the realized owner and later institutional history does not rewrite the founding plan |
...
| `site.affiliation` | Typed site ownership and material support across major settlements, frontier holdings, frontier settlements, cabins, and homesteads | `CASiteFactionLinks` nested in the owning plan/record; direct authoring, explicit affiliation events, and atomic migration only | Authoring, materialization, explicit affiliation change, and inhabited-form transition | regional/world faction references + site record → realization, threats, summaries, population, programs, habitat and native adapters | `SiteAffiliation`; `None` performs no lookup, support never becomes ownership, resident affiliation is independent, and scale transitions preserve both facts |
| `site.local-society` | Complete local Political Order, Technological Knowledge, and represented institutions for factionless or explicitly divergent sites | `CASiteLocalSocietyState` nested in the owning settlement/frontier record; detachment snapshots effective state once and later edits mutate the local owner | Detachment, direct local authoring, explicit represented change, and save/load | effective faction state or direct local composition + population Culture/Ideoligion → politics, technology, programs, provision, organizations and development | `SiteAffiliation`; an ownerless site validates complete local state, while an owned non-divergent site resolves its faction rather than duplicating it |
...
| `settlement.composition` | Authored population-group composition, independent resident affiliation, and assignment policy | Population groups and assignments inside `CARegionalSettlementRecord`; composition commands | Authoring/materialization; explicit residence or affiliation events thereafter | regional authoring → population projection, Culture/Ideoligion resolution and domestic formation | B10/B17 composition receipts; site ownership and support never infer resident affiliation, and pure authoring remains separate from runtime projection |
...
| Faction Culture, Political Order, Technological Knowledge, and represented institutions | `CAFactionStateWorldComponent` | creation/materialization, Society/component authoring, research acquisition, and represented changes |
...
| Site ownership and support | Owning settlement/frontier record through `CASiteFactionLinks` | direct authoring, explicit affiliation event, atomic adjacent-schema migration, or exact inhabited-form transition |
| Factionless local political, technological, and institutional state | Owning settlement/frontier record through `CASiteLocalSocietyState` | one detachment snapshot, direct local authoring, and explicit represented change |
...
Every implementation batch after the durable B11 boundary records:
