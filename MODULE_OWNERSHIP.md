...
Scope: active CAO source at the B16 playable social-ontology boundary
...
| `technology.knowledge` | Nine faction-owned practical domains, understand/construct/operate/maintain competencies, native research translation, known-project evidence, and optional pawn/institution/record custody | Canonical `CATechnologicalKnowledge` is nested only in `CAFactionState`; `CATechnologicalKnowledgeRuntime` is the sole availability/query boundary; founding and regional plans stage copies before faction realization | Authoring and faction creation; native research completion; distributed-mode initial distribution, usable humanlike member spawn/faction change, death, incapacity query, and game start/load initialization | faction state + native research/buildable/manufactured-item/recipe/plant definitions + custody → current consumers enumerated in `Receipts/B15/B15_TECHNOLOGICAL_KNOWLEDGE_CONSUMER_MATRIX.md` | `TechnologicalKnowledge`; standard mode reads faction state directly, distributed mode projects the same state through map/tile-accessible custody, teaching is not yet implemented, unused competency cells are forward capacity, and no settlement tier or `FactionDef` becomes a second owner |
