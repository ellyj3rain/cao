...
test("semantic replay derives the current CAO version", () => {
  ...
  assert.equal(replay.currentVersion, "1.6.0.0-alpha");
  assert.equal(CURRENT_VERSION, "1.6.0.0-alpha");
  assert.equal(replay.trace.length, 42);
...
test("version units partition A1-B15 exactly once", () => {
...
test("repository projections and identifiers match the replay", () => {
  ...
  assert.equal(result.closedBatchCount, 117);
  assert.equal(result.nextBatch, "B16");
});
