...
test("semantic replay derives the current CAO version", () => {
  ...
  assert.equal(replay.currentVersion, "1.8.0.0-alpha");
  assert.equal(CURRENT_VERSION, "1.8.0.0-alpha");
  assert.equal(replay.trace.length, 47);
...
test("version units partition A1-B20 exactly once", () => {
...
test("repository projections and identifiers match the replay", () => {
  ...
  assert.equal(result.closedBatchCount, 122);
  assert.equal(result.nextBatch, "B21");
});
