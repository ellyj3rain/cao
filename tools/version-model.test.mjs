...
test("semantic replay derives the current CAO version", () => {
  ...
  assert.equal(replay.currentVersion, "0.12.4.0-alpha");
  assert.equal(CURRENT_VERSION, "0.12.4.0-alpha");
  assert.equal(replay.trace.length, 28);
...
test("version units partition A1-B1 exactly once", () => {
...
test("repository projections and identifiers match the replay", () => {
  ...
  assert.equal(result.closedBatchCount, 103);
  assert.equal(result.nextBatch, "B2");
});
