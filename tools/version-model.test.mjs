...
test("semantic replay derives the current CAO version", () => {
  ...
  assert.equal(replay.currentVersion, "1.6.1.0-alpha");
  assert.equal(CURRENT_VERSION, "1.6.1.0-alpha");
  assert.equal(replay.trace.length, 43);
...
test("version units partition A1-B16 exactly once", () => {
...
test("repository projections and identifiers match the replay", () => {
  ...
  assert.equal(result.closedBatchCount, 118);
  assert.equal(result.nextBatch, "B17");
});
