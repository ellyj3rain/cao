...
- Batch identifiers share one namespace across letter eras. The A sequence is
  ...
  log. `B5` is closed and `B6` is next. A new letter does not create another
...
- **Batch** is the atomic chronological development record. The closed history is
  `A1` through `A102`, followed by `B1` through `B5`; `B6` is the next identifier.
...
the generated version stamps, and `VERSION_MAP.md`; validates exact A1-B5
...
gated, not hand-typed. Thirty-two evidenced version units derive
`1.2.0.0-alpha`.
