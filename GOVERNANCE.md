...
- Batch identifiers share one namespace across letter eras. The A sequence is
  ...
  log. `B6` is closed and `B7` is next. A new letter does not create another
...
- **Batch** is the atomic chronological development record. The closed history is
  `A1` through `A102`, followed by `B1` through `B6`; `B7` is the next identifier.
...
the generated version stamps, and `VERSION_MAP.md`; validates exact A1-B6
...
gated, not hand-typed. Thirty-three evidenced version units derive
`1.3.0.0-alpha`.
