...
- Batch identifiers share one namespace across letter eras. The A sequence is
  ...
  log. `B2` is closed and `B3` is next. A new letter does not create another
...
- **Batch** is the atomic chronological development record. The closed history is
  `A1` through `A102`, followed by `B1` and `B2`; `B3` is the next identifier.
...
the generated version stamps, and `VERSION_MAP.md`; validates exact A1-B2
...
gated, not hand-typed. Twenty-nine evidenced version units derive
`1.0.0.0-alpha`.
