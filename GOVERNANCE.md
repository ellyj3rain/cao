...
- Batch identifiers share one namespace across letter eras. The A sequence is
  ...
  log. `B4` is closed and `B5` is next. A new letter does not create another
...
- **Batch** is the atomic chronological development record. The closed history is
  `A1` through `A102`, followed by `B1` through `B4`; `B5` is the next identifier.
...
the generated version stamps, and `VERSION_MAP.md`; validates exact A1-B4
...
gated, not hand-typed. Thirty-one evidenced version units derive
`1.1.0.0-alpha`.
