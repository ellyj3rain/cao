...
- Batch identifiers share one namespace across letter eras. The A sequence is
  ...
  log. `B3` is closed and `B4` is next. A new letter does not create another
...
- **Batch** is the atomic chronological development record. The closed history is
  `A1` through `A102`, followed by `B1` through `B3`; `B4` is the next identifier.
...
the generated version stamps, and `VERSION_MAP.md`; validates exact A1-B3
...
gated, not hand-typed. Thirty evidenced version units derive
`1.0.1.0-alpha`.
