...
| `CULTURE_RESEARCH_CORPUS.md` | CANONICAL | Current Culture research and playable-mechanics corpus: B13 research foundation plus B16 reverse ownership audit, sources, consumers, historical feedback, and calibration limits. |
| `DATASET_GOVERNANCE.md` | CANONICAL | Public machine-learning dataset strata, provenance, acquisition, processing, privacy, IP, cohort, and model/build use-accounting boundary. Substantive examples and exact membership remain in the ignored local compliance layer. |
| `PLAYER_BASE_PATTERN_CORPUS.md` | CANONICAL, INCOMPLETE | Full-save and structured-snapshot layout methodology, evidence strata, extraction contract, and autonomous-building admission rules. Nine operator saves cover early/combat evidence; the broad Real Ruins screen defines 4,238 complete byte-unique layouts across 80 biome definitions with lineage-safe partitions. Normalized rows and exact membership remain local. Quality/capability classification, complementary mature full saves, and full-save terrain/roof decoding remain open. |
