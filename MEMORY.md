...
| `DECISION_REGISTRY.md` | CANONICAL, APPEND-ONLY | Ratified decisions DR-1 through DR-97. Later records supersede earlier decisions without rewriting them. |
...
| `VERSION` | CANONICAL, GENERATED | `1.1.0.0-alpha`, derived from the version replay. |
...
| `Batches/[A-Z]*.md` | APPEND-ONLY | The 102 closed A-sequence records and closed `B1-B4` records in one namespace. |
...
The A sequence closes at `A102`; the chronology continues through closed `B4`,
and `B5` is next. The governed batch records, chronological catalog, thematic
...
work. Threads classify related work across any letter sequence. The 31 version
units in `VERSION_MAP.md` partition A1-B4 into contiguous capability runs and
