...
| `VERSION` | CANONICAL, GENERATED | `1.0.1.0-alpha`, derived from the version replay. |
...
| `Batches/[A-Z]*.md` | APPEND-ONLY | The 102 closed A-sequence records and closed `B1-B3` records in one namespace. |
...
The A sequence closes at `A102`; the chronology continues through closed `B3`,
and `B4` is next. The governed batch records, chronological catalog, thematic
...
work. Threads classify related work across any letter sequence. The 30 version
units in `VERSION_MAP.md` partition A1-B3 into contiguous capability runs and
