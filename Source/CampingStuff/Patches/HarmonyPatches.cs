...
public partial class HarmonyPatches : Mod
{
	public HarmonyPatches(ModContentPack content) : base(content)
	{
		...
		// REMOVED: harmony.PatchAll(Assembly.GetExecutingAssembly()).
		// Camping Stuff is vendored INTO the Colonist Awareness assembly, and
		// every one of its eight targets is patched explicitly by name above -
		// it carries no [HarmonyPatch]-attributed class of its own, so this
		// call contributed nothing to Camping Stuff. What it actually did was
		// sweep up all 45 CA patch files and register them a SECOND time,
		// under this mod's Harmony id, on top of the PatchAll in ModEntry.
		// Observed live 2026-08-06: the per-cell terrain transpilers ran twice
		// (the second pass seeing already-rewritten IL and reporting a
		// fail-closed refusal), and, more seriously, every CA POSTFIX ran
		// twice - which would have made CARegionalScatterConsumePatch consume
		// two units of constituent quota per placement. One assembly, one
		// PatchAll: ModEntry's covers these classes too.
