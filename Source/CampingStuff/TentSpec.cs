...
public class TentSpec : IExposable
{
	...
	public override int GetHashCode()
	{
		if (!oldHash)
		{
			// [CA port] GenericHashLib replaced with an order-sensitive
			// combine over the same sequences - same contract, no extra
			// dependency.
			int h = 17;
			if (layout != null)
				foreach (var row in layout)
					h = h * 31 + (row != null ? row.GetHashCode() : 0);
			if (spawns != null)
				foreach (var sp in spawns)
					h = h * 31 + sp.GetHashCode();
			return h;
		}
		else
		{
