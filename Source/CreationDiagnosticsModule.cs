...
{
    // Focused creation evidence. These actions report the saved founding boundary
    // and persistent Culture history directly; they do not mutate either.
    internal static class CACreationDiagnostics
    {
        internal static string FoundingState()
        {
            ...
            CARegionalPlan region = CARegionalSetupSession
                .CurrentPlanOrNull();
            var text = new StringBuilder("[CA][Creation] founding state");
            if (founding == null)
        ...
        {
            var text = new StringBuilder("[CA][Creation] Culture history");
            if (map == null)
            ...
            CACulture culture, string indent)
        {
            ...
            text.Append("\n").Append(indent).Append("latest transition=")
                .Append(transition == null ? "none" : transition.sequence
                    + " " + transition.cause + "; subjects="
                    + (transition.changedSubjectKeys ?? "not recorded")
                    + "; predecessor="
    ...
    {
        [DebugAction("Colonist Awareness", "Founding-state census",
            actionType = DebugActionType.Action,
            allowedGameStates = AllowedGameStates.Entry)]
        private static void FoundingStateCensus()
        {
            Log.Message(CACreationDiagnostics.FoundingState());
        }

        [DebugAction("Colonist Awareness", "Culture-history census",
            actionType = DebugActionType.Action,
            allowedGameStates = AllowedGameStates.PlayingOnMap)]
        private static void CultureHistoryCensus()
        {
            Log.Message(CACreationDiagnostics.CultureHistory(
                Find.CurrentMap));
        }
    }
}
